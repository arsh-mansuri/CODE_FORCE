...
export function ProfilePage({ user, onUserUpdate, onEditMedia, onLogout }: ProfilePageProps) {
  ...
  if (lifestyle?.sleep_schedule) chips.push({ icon: lifestyle.sleep_schedule === 'early_bird' ? 'wb_sunny' : 'bedtime', label: words(lifestyle.sleep_schedule) });
  ...
  if (lifestyle?.smokes != null) chips.push({ icon: lifestyle.smokes ? 'smoking_rooms' : 'smoke_free', label: lifestyle.smokes ? 'Smoker' : 'Non-smoker' });
